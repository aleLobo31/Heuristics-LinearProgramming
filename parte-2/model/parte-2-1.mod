...
var c{i in BUSES, k in SLOTS} binary; # Variable de asignación

# Defino la función obketivo
minimize COSTE_TOTAL:
    (sum{i in BUSES, k in SLOTS} c[i,k] * DISTANCIAS[i] * COSTE_ASIGNACION)+(sum{i in BUSES} ((1 - sum{k in SLOTS} c[i,k]) * PASAJEROS[i] * COSTE_OPORTUNIDAD));
    
# Defino las restricciones de mi problema
s.t. MAX_UN_SLOT_POR_BUS{k in SLOTS}:
# Para cada slot k compruebo que la suma de todas las variable de asignación sea <= 1
    sum{i in BUSES} c[i,k] <= 1;
s.t. MAX_UN_BUS_POR_SLOT{i in BUSES}:
# Para cada bus i compruebo que la suma de todas las variable de asignación sea <= 1
    sum{k in SLOTS} c[i,k] <= 1;
...
printf "Bus → Slot\n";
display {i in BUSES, k in SLOTS: c[i, k] = 1} c[i, k];
printf("********************************************\n");
